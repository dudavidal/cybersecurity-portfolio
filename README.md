<p align="center" style="font-size: 1.5rem;">
  🌸 👩‍💻 🤓
</p>

# 👩‍💻 Portfólio de Cibersegurança 

 Sou estudante de Computação e estou iniciando minha jornada na área de cibersegurança.

Este portfólio foi criado com o objetivo de documentar meu processo de aprendizado e acompanhar minha evolução na área. Comecei meus estudos a partir dos fundamentos, principalmente redes de computadores, e estou avançando por meio de prática com laboratórios e desafios reais.

Este portfólio representa minha evolução na área de cibersegurança, desde os primeiros passos até níveis mais avançados.


## 🎯 Objetivo

* Documentar write-ups de máquinas e desafios
* Demonstrar meu raciocínio técnico e processo de exploração
* Acompanhar minha evolução em cibersegurança

## 🧠 Atualmente estudando

* Fundamentos de redes
* Enumeração de serviços
* Exploração de vulnerabilidades web
* Escalada de privilégios em Linux

## 📁 Projetos

### 🔐 TryHackMe

* Root Me → [ver write-up](./TryHackMe/Root-me/Write-up.pdf)
* Pickle Rick→ [ver write-up](./TryHackMe/Pickle-Rick/Write-up.pdf)
* Simple Ctf → [ver write-up](./Simple-Ctf/Write%20up.pdf)  
* Chocolate Factory  → [ver write-up](./TryHackMe/Chocolate-Factory/Write-up.pdf)

### 🔐 Crack de Senhas

* Hashcat (nível iniciante) → [Ver atividade](./Criptografia_e_Cracking/CTF_hashes.pdf)
* Crack the Hash → [ver write-up](./Criptografia_e_Cracking/WriteUpCracktheHash.pdf)

### 🌐 Redes

* Fudamentos de rede→ [ver projeto](./Redes/fundamentos/Lista-exercicios-Teoria.pdf)
* Traceroute Lab → [ver projeto](./Redes/Traceroute/Funcionamento-Tracerouter.pdf)
* Sistema de chat → [ver projeto](./Redes/SistemaChat/)  

## 🎓 Certificados

- Introdução à Cibersegurança → https://www.credly.com/badges/398ce151-10cd-4f00-8ba5-8ba8b8939540/public_url
- Noções básicas de redes → https://www.credly.com/badges/46ce491c-d367-4545-9575-5d218c68b895/public_url 
